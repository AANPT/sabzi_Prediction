...
# Load the dataset
data = pd.read_csv('C:/Users/ns212/Desktop/ML/ML/vegetable_prices.csv')
...
if st.button("Predict Price"):
    ...
    if vegetable_data.empty:
        ...
        last_sequence = target_scaled[-sequence_length:].reshape(
            1, sequence_length, 1)
        predicted_price_scaled = model.predict(last_sequence)
        predicted_price = scaler.inverse_transform(predicted_price_scaled)

        st.success(
            f"Predicted Price for Today ({vegetable_to_predict}): {predicted_price[0][0]}")
